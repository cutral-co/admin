...
{
    public function index_(Request $request, $redirect = null)
    {
        try {
            $options = [
    ...
    public function index(Request $request, $redirect = null)
    {
        try {
            ...
            try {
                ...
                if ($request->type === 'SW_EmitirCompPagoDeudaVenc' || $request->type === 'SW_EmitirCompPagoNoVenc') {
                    // return $this->getXMLRequestEmitirComp($request);
                    $response =  $client->__doRequest($this->getXMLRequestEmitirComp($request), $options['location'], 'urn:uSWConsultaTributariaIntf-ISWConsultaTributaria#SW_ConsultarDeuda', 1);
                } else {
                    if (isset($request->params['pTipoImponible']) && $request->params['pTipoImponible'] == 'V' && !isset($request->params['cod_veh'])) {
                        $xml =  $this->getXMLRequestVehiculo($request);
                        //return $xml;
                        $response =  $client->__doRequest($xml, $options['location'], 'urn:uSWConsultaTributariaIntf-ISWConsultaTributaria#SW_ConsultarDeuda', 1);
                    } else {
                        $response =  $client->__doRequest($this->getXMLRequest($request), $options['location'], 'urn:uSWConsultaTributariaIntf-ISWConsultaTributaria#SW_ConsultarDeuda', 1);
                    }
                }
                ...
                if (isset($matches[1])) {
                    ...
                    if (($request->type === 'SW_EmitirCompPagoDeudaVenc' ||$request->type === 'SW_EmitirCompPagoNoVenc')  && !$request->method) {
                        return sendResponse($xmlContent);
                    }
                    // Cargar el XML limpio
                    $xmlObject = (object)simplexml_load_string($xmlContent);
    ...
    private function getXMLRequest(Request $request)
    {
        ...
        if ($request->type == 'SW_ImponibleXCUIT2') {
            if (strlen($attributes['pCUIT'])) {
                $attributes['pCUIT'] = substr($attributes['pCUIT'], 0, -1);
            }
        }
    ...
    private function getXMLRequestVehiculo(Request $request)
    {
        $attributes = $request->params;
        $attributes['Key'] = env('KEY_TRIBUTARIA');

        $patente =  $attributes['pNroImponible'];
        ...
        >
            <soapenv:Body>
                <urn:SW_VehiculoXDominio soapenv:encodingStyle='http://schemas.xmlsoap.org/soap/encoding/'>
                    <pDominio xsi:type='xsd:string'>$patente</pDominio>
                    <Key xsi:type='xsd:string'>11111111110000000000</Key>
                </urn:SW_VehiculoXDominio>
            </soapenv:Body>
        </soapenv:Envelope>");
        ...
        $response =  $client->__doRequest($xml->asXML(), $options['location'], 'urn:uSWConsultaTributariaIntf-ISWConsultaTributaria#SW_ConsultarDeuda', 1);

        preg_match('/<return xsi:type="xsd:string">(.*?)<\/return>/s', $response, $matches);

        $tributoId =  $matches[1];

        $fecha = $attributes['pFechaAct'];
            ...
            >
                <soapenv:Body>
                    <urn:SW_ConsultarDeuda soapenv:encodingStyle='http://schemas.xmlsoap.org/soap/encoding/'>
                        <pTipoImponible xsi:type='xsd:string'>V</pTipoImponible>
                        <pNroImponible xsi:type='xsd:string'>$tributoId</pNroImponible>
                        <pFechaAct xsi:type='xsd:string'>$fecha</pFechaAct>
